...
public class DriveANN{
	...
	public double alpha;
	List<DriveLayer> layers = new List<DriveLayer>();

	public DriveANN(int nI, int nO, int nH, int nPH, double a)
	{
		numInputs = nI;
		numOutputs = nO;
		numHidden = nH;
		...
		if(numHidden > 0)
		{
			layers.Add(new DriveLayer(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new DriveLayer(numNPerHidden, numNPerHidden));
			}

			layers.Add(new DriveLayer(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new DriveLayer(numOutputs, numInputs));
		}
	}
	...
	{
		string weightStr = "";
		foreach(DriveLayer l in layers)
		{
			foreach(DriveNeuron n in l.neurons)
			{
