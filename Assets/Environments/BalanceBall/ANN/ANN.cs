...
public class BalanceBallANN{
	...
	public double alpha;
	List<BalanceBallLayer> layers = new List<BalanceBallLayer>();

	public BalanceBallANN(int nI, int nO, int nH, int nPH, double a)
	{
		numInputs = nI;
		numOutputs = nO;
		numHidden = nH;
		...
		if(numHidden > 0)
		{
			layers.Add(new BalanceBallLayer(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new BalanceBallLayer(numNPerHidden, numNPerHidden));
			}

			layers.Add(new BalanceBallLayer(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new BalanceBallLayer(numOutputs, numInputs));
		}
	}
	...
	{
		string weightStr = "";
		foreach(BalanceBallLayer l in layers)
		{
			foreach(BalanceBallNeuron n in l.neurons)
			{
