...
public class ANNPong{
	...
	public double alpha;
	List<LayerPong> layers = new List<LayerPong>();
	...
	public ANNPong(int nI, int nO, int nH, int nPH, double a)
	{
		...
		if(numHidden > 0)
		{
			layers.Add(new LayerPong(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new LayerPong(numNPerHidden, numNPerHidden));
			}

			layers.Add(new LayerPong(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new LayerPong(numOutputs, numInputs));
		}
	}
	...
	{
		string weightStr = "";
		foreach(LayerPong l in layers)
		{
			foreach(NeuronPong n in l.neurons)
			{
